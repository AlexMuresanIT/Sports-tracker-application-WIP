...
public class OutdoorRunningProducer {
  ...
  public void sendNewOutdoorRunningRecord(final OutdoorRunning outdoorRunning) {
    ...
    try {
      logger.info(
          "Send new outdoor running record for user with email {}", outdoorRunning.getUserEmail());
      kafkaTemplate.send(
          NEW_OUTDOOR_RUNNING_RECORD, generateMessageKey(outdoorRunning), outdoorRunning);
    } catch (Exception e) {

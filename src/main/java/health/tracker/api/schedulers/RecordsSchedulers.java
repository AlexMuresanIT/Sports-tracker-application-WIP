...
public class RecordsSchedulers {
  ...
  public RecordsSchedulers(
      final OutdoorRunningService outdoorRunningService, final UserService userService) {
  ...
  public void recordsCounterForAnUser() {
    final var userEmail = getRandomEmail();
    final var recordsCounter =
        outdoorRunningService.getAllOutdoorRunningRecordsOfAnUser(userEmail).size();
    if (recordsCounter > 0) {
